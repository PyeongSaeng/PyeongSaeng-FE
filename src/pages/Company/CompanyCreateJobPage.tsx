...
export default function CompanyCreateJobPage({
  ...
  const lastUploadedName =
    draft.jobPostImageList?.[draft.jobPostImageList.length - 1]
      ?.originalFileName ?? '';
  ...
  return (
    <div
      className="w-full h-full flex flex-col items-center overflow-y-auto scrollbar-hide"
    ...
    >
      ...
      <div className="flex flex-row mt-[26px] gap-[24px]">
        ...
        <div
          className={`w-[231px] h-[45px] transition-colors ${dragging ? 'bg-blue-50' : 'bg-white'}`}
        >
          <ImageUploadButton
            imageFile={imageFile}
        ...
        <div className="w-[251px] h-[120px] pl-[12px] flex flex-col gap-[10px] items-center justify-center">
          <AddressSearchInput
            ...
            onChangeDetail={(detail) =>
              onChangeDraft({ detailAddress: detail })
            }
            onChangeRoadAddress={(roadAddress) =>
              onChangeDraft({ roadAddress })
            }
          />
        </div>
      </div>
